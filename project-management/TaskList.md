# Tâches — Scorpanion

## Frontend

- [ ] **Navigation** — routes (scoreboard, historique, statistiques, login, register)
- [ ] **Écrans** — pages login / register
- [x] **Écrans** — pages d'accueil
- [x] **Écrans** — Création d'un scoreboard
- [x] **Écrans** — Affichage d'un scoreboard
- [ ] **Scoreboard** — Gestion d'une partie / fin de partie
- [ ] **Historique** — liste et consultation des parties terminées
- [ ] **Statistiques** — vue agrégée (parties jouées, scores, victoires)
- [ ] **Appels API** — clients frontend pour game, gameResult, scoreboard, stats

## Backend

- [ ] **Entités** — modèles de domaine, mapping BDD
- [ ] **DAL** — accès données, repositories
- [ ] **Navigation** — routes API, contrôleurs, cohérence des endpoints
- [ ] **Gestion d'une Game** — logique métier, cycle de vie d’une partie
- [ ] **Enregistrement d'un GameResult** — persistance du résultat, validation
- [ ] **Gestion d'un Scoreboard** — scores, classement
